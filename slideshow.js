...
textSp_dom = document.getElementById("textLrn");
textEn_dom = document.getElementById("textKnw");
textSpEx_dom  = document.getElementById("textLrnEx");
textEnEx_dom  = document.getElementById("textKnwEx");
...
function loadLesson() {
	...
	//img_dom
	textSp_dom.innerText     = cards[card_id].lrn;
	textEn_dom.innerText     = cards[card_id].knw;
	ex_numbers  = cards[card_id].lrn_ex.length;
	ex_no       = random(0,ex_numbers-1);
	console.log("ex no is ", ex_no);
	textSpEx_dom.innerText   = cards[card_id].lrn_ex[ex_no];
	textEnEx_dom.innerText   = cards[card_id].knw_ex[ex_no];
...
function nextCard(prev=false) {
	if (cards_deck_to_learn.length > 1){
		...
		textSp_dom.innerHTML   = cards[card_id].lrn;
		textEn_dom.innerHTML   = cards[card_id].knw;
		ex_numbers  = cards[card_id].lrn_ex.length;
		ex_no       = random(0,ex_numbers-1);
		console.log("ex no is ", ex_no);
		textSpEx_dom.innerHTML   = cards[card_id].lrn_ex[ex_no];
		textEnEx_dom.innerHTML   = cards[card_id].knw_ex[ex_no];
